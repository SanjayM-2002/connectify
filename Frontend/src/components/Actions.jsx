...
const Actions = ({ post }) => {
  ...
  const handleLikeAndUnlike = async () => {
    ...
    try {
      ...
      // console.log('Data from like request is: ', data);
      if (data.error) {
        console.log('Error in like request is: ', data.error);
        showToast('Error', data.error, 'error');
        return;
      }

      if (!liked) {
        // add the _id og the current user to the post.likes array
        ...
        // remove the _id og the current user to the post.likes array
        const updatedPosts = posts.map((p) => {
          if (p._id === post._id) {
            return {
              ...p,
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      const data = await res.json();
      // console.log('Data from reply request is: ', data);
      if (data.error) {
        console.log('Error in reply request is: ', data.error);
        showToast('Error', data.error, 'error');
        return;
      }
...
const LikeSVG = ({ liked, handleLikeAndUnlike }) => {
  return (
    <svg
      aria-label='Like'
      color={liked ? 'rgb(237, 73, 86)' : ''}
